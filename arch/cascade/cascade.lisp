...
(export '(cascade-network make-cascade-network cascade-eval))
...
(defun cascade-eval-output (cascade &optional values)
  ...
  (let ((out-weights (cascade-out-weights cascade))
	...
	(%setf (%fvref (cascade-outputs cascade) i) val)
	(incf off (dim0 conns)))))
  (when (cascade-if-nonlinear-outputs cascade)
    (activation (cascade-outputs cascade) (cascade-act-fn cascade)))
...
(defun cascade-out-test-part-size (test-part cascade npats)
  (if (eq test-part :auto)
      (optimal-test-part-size (array-total-size (cascade-out-weights cascade)) npats)
      test-part))

(defun cascade-cand-test-part-size (test-part cascade npats)
  (if (eq test-part :auto)
      (/ (optimal-test-part-size (1+ (dim0 (cascade-values cascade))) npats) 2.0)
      test-part))
