...
(defun centering (patterns means)
  (patterns-dispatch (patterns)
		     ...
		     (centering-input (first patterns) means)
		     ...
		     (map nil #'(lambda (i) (centering-input i means)) patterns)
		     (map nil #'(lambda (p) (centering-input (first p) means)) patterns))
  patterns)

(defun undo-centering (patterns means)
  (patterns-dispatch (patterns)
		     (undo-centering-input patterns means)
		     (undo-centering-input (first patterns) means)
		     (dotimes (i (dim0 patterns) patterns)
		       (setf (row patterns i)
		     ...
		     (map nil #'(lambda (i) (undo-centering-input i means)) patterns)
		     (map nil #'(lambda (p) (undo-centering-input (first p) means)) patterns))
  patterns)
