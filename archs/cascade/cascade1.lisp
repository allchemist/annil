...
(export '(cascade-network cascade-act-fn cascade-if-nonlinear-outputs cascade-ninputs cascade-noutputs cascade-nhidden
	  ...
	  make-cascade-network cascade-ninputs cascade-insert-node cascade-install-input cascade-node-conns
...
(defun make-cascade-network (input-size output-size act-fn
  ...
  (let ((cascade
	 (make-instance 'cascade-network
			...
			:out-weights (make-random-matrix (* output-size (1+ input-size))))))
    (%setf (%fvref (cascade-values cascade) input-size) 1.0)
  ...
  (symbol-macrolet ((w (cascade-weights cascade))
	...
	(dotimes (o nouts)
	  (let ((conns (dim0 (svref oc o))))
	    ...
	    (setf (aref new-ow (+ off conns o)) (random-value 1.0)); (random-elt ow))
	    (incf off conns)))
	(setf ow new-ow))
      (dotimes (o nouts)
