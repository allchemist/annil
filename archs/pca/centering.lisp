...
(defun centering (patterns means)
  (patterns-dispatch (patterns)
		     ...
		     (%build-pattern (centering-input (first patterns) means) (second patterns)
				     (when (= (length patterns) 3) (last patterns)))
...
(defun undo-centering (patterns means)
  (patterns-dispatch (patterns)
		     ...
		     (%build-pattern (undo-centering-input (first patterns) means) (second patterns)
				     (when (= (length patterns) 3) (last patterns)))
		     (dotimes (i (dim0 patterns) patterns)
		       (setf (row patterns i)
