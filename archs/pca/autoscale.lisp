...
(defun undo-autoscale (patterns means norms)
  (patterns-dispatch (patterns)
		     ...
		     (list (undo-autoscale-input (copy (first patterns)) means norms)
			   (second patterns))
		     (let ((inputs (make-matrix-like patterns)))
		       (dotimes (i (dim0 inputs))
			 (setf (row inputs i)
		     ...
		     (mapcar #'(lambda (p)
				 (list (undo-autoscale-input (copy (first p)) means norms) (second p)))
			     patterns)))
...
(defun autoscale (patterns means norms)
  (patterns-dispatch (patterns)
		     ...
		     (list (autoscale-input (first patterns) means norms)
			   (second patterns))
...
(defun undo-autoscale (patterns means norms)
  (patterns-dispatch (patterns)
		     ...
		     (list (undo-autoscale-input (first patterns) means norms)
			   (second patterns))
		     (dotimes (i (dim0 patterns) patterns)
		       (setf (row patterns i)
			     (undo-autoscale-input (row patterns i) means norms)))
