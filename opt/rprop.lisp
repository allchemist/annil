...
(defun rprop (w slope-fn err-fn train-patterns test-patterns params)
  (let ((restarts (or (param params :restarts) 0))
    ...
    (dotimes (i (1+ restarts))
	  ...
	  (rprop-single w slope-fn err-fn train-patterns test-patterns params)
	(setf err cur-err)
...
(defun rprop-single (w slope-fn err-fn train-patterns test-patterns init-params)
  ...
  (let ((s (make-matrix-like w))
    ...
    (dotimes (e (param params :epochs))
      ...
      ;; eval slopes
      (setf err-1 err
	    err (/ (nth-value 1 (funcall slope-fn w s)) (num-patterns train-patterns)))
      (m/c s (* err (num-patterns train-patterns)))
      ;; for each weight
      (dotimes (i (array-total-size w))
	...
	(let ((sv (row-major-aref s i))
      ...
      (when test-patterns
	    (setf test-err-1 test-err
		  test-err (/ (funcall err-fn w) (num-patterns test-patterns))))      
      (when (>= verbosity 3)
	(if test-patterns
    ...
    (when (>= verbosity 1)
      (if test-patterns
	  (info "Final result: ~A, over test patterns: ~A~%" err test-err)
	  (info "Final result: ~A~%" err)))
    (values w err epochs-passed)))
