...
(defmethod encode ((codec pca-codec) patterns)
  (let ((-P (pca-loadings codec)))
    (patterns-dispatch (patterns)
		       ...
		       (%build-pattern (gemv -P patterns :transa :trans) (copy (second patterns))
				       (when (= (length patterns) 3) (third patterns)))
		       (gemm patterns -P)
		       (%build-pattern (gemm (first patterns) -P) (copy (second patterns))
				       (when (= (length patterns) 3) (copy (third patterns))))
		       (map 'simple-vector #'(lambda (i) (gemv -P i :transa :trans)) patterns)
		       (map 'simple-vector #'(lambda (p) (%build-pattern (gemv -P (first p) :transa :trans)
									 (copy (second p))
									 (when (= (length p) 3) (third p))))
			    patterns))))
...
(defmethod decode ((codec pca-codec) patterns)
  (let ((-P (pca-loadings codec)))
    (patterns-dispatch (patterns)
		       (gemv -P patterns)
		       (%build-pattern (gemv -P patterns) (copy (second patterns))
				       (when (= (length patterns) 3) (third patterns)))
		       (gemm patterns -P :transb :trans)
		       (%build-pattern (gemm (first patterns) -P :transb :trans) (copy (second patterns))
				       (when (= (length patterns) 3) (copy (third patterns))))
		       (map 'simple-vector #'(lambda (i) (gemv -P i)) patterns)
		       (map 'simple-vector #'(lambda (p) (%build-pattern (gemv -P (first p)) (copy (second p))
							       (when (= (length p) 3) (third p))))
			    patterns))))
